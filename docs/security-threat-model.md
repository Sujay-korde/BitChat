...
| **Man-in-the-Middle (Key Exchange)** | **NOT PROTECTED** | The `KEY_EXCHANGE` protocol sends ephemeral X25519 public keys in plaintext. The exchange is completely unauthenticated, permitting a classic active MITM attack. |
...
| Peer identity authenticated | **NOT PROTECTED** | No identity keys or signatures during DH exchange. |
| MITM resistance | **NOT PROTECTED** | Unauthenticated `KEY_EXCHANGE`. |
...
* **Compatibility**: Interoperable between Python `cryptography.hazmat` and TypeScript `@noble/curves`. Uses raw 32 bytes encoded in Base64 for transmission.
...
* "MITM-proof" or "Peer Authenticated"
